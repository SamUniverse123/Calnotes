...
import {
  ...
  findProduct,
  getFile,
} from "@/app/(Admin)/admin/_actions/product";
...
export default async function EditProductsForm(name: string) {
  const product = await findProduct(name);
  const files = await getFile(product?.filePath!);
